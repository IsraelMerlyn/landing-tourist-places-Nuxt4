...
const handleEliminar = async (id: number) => {
  ...
  if (result.isConfirmed) {
    try {
      ...
      Swal.fire('¡Borrado!', 'El destino ha sido eliminado.', 'success')
    } catch (error: any) {
      Swal.fire('Error', error.message, 'error')
    }
  }
  ...
  <div class="min-h-screen bg-slate-50 p-6 lg:p-10 font-sans relative">
    ...
    <header class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4 mb-10">
      ...
      <div class="flex gap-4">
        <button @click="logout" class="px-5 py-2.5 rounded-xl border border-gray-200 text-gray-600 font-bold text-sm hover:bg-white hover:text-red-500 transition-colors shadow-sm">
          Salir
        </button>
        <button @click="abrirModal()" class="px-6 py-2.5 rounded-xl bg-emerald-600 text-white font-bold text-sm hover:bg-emerald-500 shadow-lg shadow-emerald-600/20 transition-transform hover:-translate-y-0.5 flex items-center gap-2">
    ...
    <main class="max-w-7xl mx-auto">
      ...
      <div v-else-if="destinos.length === 0" class="bg-white rounded-[2rem] p-12 text-center border border-dashed border-gray-300">
        ...
        <p class="text-gray-500 mb-6">Agrega el primero.</p>
      </div>
          ...
          <table class="w-full text-left text-sm text-gray-500">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="destino in destinos" :key="destino.id" class="hover:bg-gray-50/80 transition-colors group">
                ...
                <td class="px-6 py-4">
                  <span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-100">{{ destino.categoria }}</span>
                </td>
                <td class="px-6 py-4 font-medium text-gray-900">{{ destino.precio }}</td>
                <td class="px-6 py-4 text-right">
                  <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button @click="abrirModal(destino)" class="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Editar">
                    ...
                    <button @click="handleEliminar(destino.id)" class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Eliminar">
    ...
    <div v-if="showModal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-40 flex items-center justify-center p-4">
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in-up">
        ...
        <div class="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center">
          <h3 class="font-bold text-gray-800 text-lg">{{ isEditing ? 'Editar Destino' : 'Nuevo Destino' }}</h3>
        ...
        <form @submit.prevent="handleGuardar" class="p-6 space-y-4">
          ...
          <div class="pt-4 flex justify-end gap-3">
            ...
            <button type="submit" :disabled="saving" class="px-6 py-2 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-500 rounded-lg shadow-lg shadow-emerald-600/20 disabled:opacity-50">
              {{ saving ? 'Guardando...' : (isEditing ? 'Actualizar' : 'Guardar Destino') }}
            </button>
          </div>
        </form>
      </div>
